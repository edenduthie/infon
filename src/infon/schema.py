...
class AnchorSchema:
    ...
    def add_anchor(self, name: str, type_: str,
                   tokens: list[str] | None = None,
                   parent: str | None = None,
                   **extra) -> bool:
        """Add a new anchor in place. Returns False if name already exists."""
        if name in self.anchors:
            return False
        info = {"type": type_, "tokens": tokens or [name]}
        if parent:
            info["parent"] = parent
        info.update(extra)
        self.anchors[name] = info
        self.names.append(name)
        self.types[name] = type_
        if type_ not in self.anchor_types:
            self.anchor_types = sorted(set(self.types.values()))
        self.by_type[type_].append(name)
        if parent:
            self._parent[name] = parent
            self._children[parent].append(name)
        return True

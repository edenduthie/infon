...
def mass_from_polarity(infon: Infon) -> MassFunction:
    ...
    Uses a smoother confidence→strength curve so low-confidence
    infons stay mostly θ. This is the key calibration knob: if the
    extraction was marginal, we should NOT lean confidently toward
    SUPPORTS just because no negation cue was present.
    """
    # Low-confidence infons (c<0.3) get almost all their mass on theta;
    # high-confidence ones commit proportionally.
    c = infon.confidence
    if c < 0.3:
        # Very marginal extraction — mostly ignorance
        evidence_weight = 0.15 * (c / 0.3)   # at c=0, w=0; at c=0.3, w=0.15
    else:
        evidence_weight = 0.15 + 0.35 * min((c - 0.3) / 0.7, 1.0)
        # at c=0.3, w=0.15; at c=1.0, w=0.50 (old cap behaviour)

    if infon.polarity == 0:
        # Explicit negation detected — evidence weighs toward REFUTES
        return MassFunction(
            refutes=evidence_weight * 0.9,
            uncertain=evidence_weight * 0.1,
            theta=1.0 - evidence_weight,
        )
    else:
        # Affirmed — most evidence_weight goes to SUPPORTS, but a
        # small slice still goes to θ so low-confidence affirmations
        # don't become high-S ones.
        return MassFunction(
            supports=evidence_weight,
            theta=1.0 - evidence_weight,
        )
...
def mass_from_evidentiality(infon: Infon) -> MassFunction:
    """Evidence source 5: evidentiality class of the originating sentence.

    Three classes produce three mass profiles:
        assertion   → lean SUPPORTS (declarative, unhedged)
        hedge       → lean θ (ignorance: we're being told it might hold)
        attribution → mixed SUPPORTS + θ (someone else claimed it; we
                      don't know whether the source is reliable yet)

    Falls back to vacuous mass if the infon has no evidentiality tag.
    """
    label = getattr(infon, "evidentiality", None) or "assertion"
    if label == "assertion":
        return MassFunction(supports=0.55, theta=0.4, uncertain=0.05)
    if label == "hedge":
        return MassFunction(theta=0.8, uncertain=0.15, supports=0.05)
    if label == "attribution":
        return MassFunction(supports=0.30, theta=0.6, uncertain=0.1)
    return MassFunction(theta=1.0)


def mass_from_modality(infon: Infon) -> MassFunction:
    """Evidence source 6: modality of the originating sentence.

    Three classes:
        certain     → lean SUPPORTS (the sentence asserts factual claim)
        possible    → lean θ (modal-qualified, we don't know yet)
        obligation  → lean UNCERTAIN (something SHOULD hold, but may not)

    Falls back to vacuous mass if the infon has no modality tag.
    """
    label = getattr(infon, "modality", None) or "certain"
    if label == "certain":
        return MassFunction(supports=0.5, theta=0.45, uncertain=0.05)
    if label == "possible":
        return MassFunction(theta=0.75, uncertain=0.2, supports=0.05)
    if label == "obligation":
        return MassFunction(uncertain=0.5, theta=0.45, supports=0.05)
    return MassFunction(theta=1.0)

...
We evaluate Cognition against five baselines on three public fact-verification
...
\begin{itemize}
  ...
  \item \textbf{SciFact}~\cite{wadden2020scifact}: Scientific claim
    ...
    \textsc{refutes}, \textsc{nei}.  We use the test split.
\end{itemize}
...
\begin{enumerate}
  \item \textbf{Cognition+GNN}: \texttt{InfonStore} with MCTS search and
    the sheaf GNN chain-verdict head as terminal scorer.
  \item \textbf{Cognition-symbolic}: \texttt{InfonStore} with MCTS and DS
  ...
  \item \textbf{LLM zero-shot}: \texttt{claude-sonnet-4-6} at temperature
...
\textbf{Hypothesis H1}: MCTS traversal in Cognition outperforms flat
...
Cognition+GNN achieves 74.0\% and flat retrieval achieves 68.0\%, a gap
of 6 percentage points.  The gap widens substantially at greater depths: at
3 hops, Cognition+GNN reaches 70.0\% versus 55.0\% for flat retrieval
(15 pp gap); at 4 hops, 66.0\% versus 42.0\% (24 pp gap).
Cognition-symbolic follows closely: 65.0\% at 3 hops and 58.0\% at 4 hops,
...
\begin{table}[t]
  ...
  \caption{HoVer accuracy by chain depth (polarity accuracy, \%).
    Cognition+GNN maintains high accuracy as depth increases while flat
    retrieval degrades.}
  \label{tab:h1_accuracy}
  \begin{tabular}{lcccc}
    \toprule
    \# Hops & Cog+GNN & Cog-sym & Flat retr. & Sym. floor \\
    \midrule
...
\paragraph{Finding.}
H1 is \textbf{supported}.  Cognition+GNN exceeds flat retrieval by
...
for Cognition-symbolic, confirming that it derives from structured traversal
...
\textbf{Hypothesis H2}: Cognition's $m(\Theta)$ rises significantly for
...
Cognition+GNN assigns $m(\Theta) = 0.90$ to NEI claims while assigning only
0.13 and 0.17 to \textsc{supports} and \textsc{refutes} respectively.
Cognition-symbolic shows a similarly large spread (NEI: 0.88, others:
...
\begin{table}[t]
  \centering
  \caption{Mean $m(\Theta)$ by ground-truth class on AVeriTeC.  Cognition
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    Cognition+GNN       & 0.13 & 0.17 & 0.90 \\
    Cognition-symbolic  & 0.15 & 0.18 & 0.88 \\
...
Cognition+GNN, 0.93 for Cognition-symbolic, and 0.52 for LLM zero-shot.
The false-positive endorsement rate on NEI claims is 3\% for Cognition+GNN,
4\% for Cognition-symbolic, and 24\% for LLM zero-shot.
...
\begin{figure}[t]
  ...
  \caption{Distribution of $m(\Theta)$ by ground-truth class on AVeriTeC.
    Cognition systems (top two panels) show a bimodal separation; the LLM
...
\paragraph{Finding.}
H2 is \textbf{supported}.  Cognition's vacuous-mass mechanism cleanly
separates NEI from evidenced claims: $\rho = 0.95$ for Cognition+GNN
...
\textbf{Hypothesis H3}: Cognition's DS mass is better calibrated (lower
...
a held-out portion of the dev split.  For Cognition systems, the DS mass
$m_S$ is used as the confidence signal.

\paragraph{Results.}
Table~\ref{tab:h3_calibration} shows calibration metrics.  Cognition+GNN
achieves ECE~$= 0.05$ and AURC~$= 0.15$, outperforming Cognition-symbolic
(ECE~$= 0.08$, AURC~$= 0.20$) and the NLI classifier (ECE~$= 0.12$,
AURC~$= 0.30$).  The Brier score follows the same ordering: 0.12 for
Cognition+GNN vs.\ 0.22 for the NLI classifier.
...
\begin{table}[t]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    Cognition+GNN       & 0.05 & 0.12 & 0.15 \\
    Cognition-symbolic  & 0.08 & 0.16 & 0.20 \\
...
\begin{figure}[t]
  ...
  \caption{Reliability diagrams on SciFact.  Cognition systems (left two
...
\begin{figure}[t]
  ...
  \caption{Accuracy vs.\ AURC Pareto frontier across all systems on the
    cross-dataset aggregate.  Cognition+GNN dominates all baselines on both
    dimensions simultaneously.}
  \label{fig:pareto}
\end{figure}

\paragraph{Finding.}
H3 is \textbf{supported}.  Cognition+GNN's ECE of 0.05 is less than half
...
on the aggregate panel.  Cognition+GNN achieves the best accuracy (70.0\%)
and the best AURC (0.15) simultaneously, placing it on the Pareto frontier
and dominating all baselines on both axes.  Cognition-symbolic occupies the

...
distilled SPLADE variant with 4.4M parameters (17~MB).  The model is
distributed by its author on the Hugging Face Hub and was trained on
Microsoft's MS-MARCO passage corpus, which is released for
non-commercial research use only; users embedding Infon in commercial
products should review the upstream model and dataset terms.  The encoder
runs on CPU in under 2~seconds cold-start and requires no GPU or external
API.  Each ingested document is encoded into a

...
We present \textbf{Cognition}, a deployable knowledge graph reasoner that
fills this gap. Cognition is not a neural language model; it is a symbolic
...
\begin{enumerate}
  ...
  \item \textbf{Dempster--Shafer mass with first-class} $\boldsymbol{m(\Theta)}$.
    ...
    frame $\{S, R, U, \Theta\}$ (Supports, Refutes, Uncertain, Vacuous).
    When the corpus provides no evidence, $m(\Theta) \to 1.0$ instead of a
    confident wrong answer~\cite{shafer1976mathematical}.
...
Section~\ref{sec:architecture} describes Cognition's four-layer architecture

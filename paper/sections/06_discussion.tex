...
At 4-hop depth, Cognition+GNN achieves 66.0\% accuracy versus flat retrieval's
...
branching factor uniformly.  Cognition-symbolic's parallel advantage (23 pp
...
Cognition+GNN assigns mean $m(\Theta) = 0.90$ to NEI claims and only 0.13
...
(3\% vs.\ 24\% for Cognition+GNN vs.\ LLM) is a direct consequence of this
structural difference.

\paragraph{H3 — DS calibration.}
Cognition+GNN achieves ECE~$= 0.05$ versus the NLI classifier's ECE~$=
...
\paragraph{Epic~02 encoder-collapse null result.}
During the evaluation of the reference\_v2 system (the predecessor to
Cognition) using synthetic text templates, all 7 evaluation cells produced
exactly 0.332 accuracy — chance level for a three-class problem.  Root-cause
analysis revealed that BERT's entity tokenisation collapsed on templated
patterns: distinct named entities in the template generated identical
contextual embeddings, making every retrieval key indistinguishable.  This
invalidated the synthetic evaluation harness for that system.  The v4
system (Cognition) avoids this failure in two ways: SPLADE-tiny uses sparse
activations over the full vocabulary rather than dense contextual
embeddings, so distinct tokens produce distinct sparse codes regardless of
surrounding template structure; and the current evaluation uses real
Wikipedia, AVeriTeC, and SciFact text rather than template-generated
synthetic corpora.  We report this null result in the interest of
reproducibility and to warn practitioners against evaluating dense
retrievers on templated text.
...
\begin{itemize}
  ...
  \item \textbf{Multimodal infons.}  The cassette format encodes
    ...
    (via structured query expansion) would allow Cognition to reason over

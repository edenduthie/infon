...
_JSON_FIELDS = {
    ...
    "locations", "temporal_refs", "metrics",
}
...
class LocalStore:
    ...
    def __init__(self, db_path: str = "cognition.db"):
    ...
    def init(self) -> None:
        ...
        # Migrate: add metrics column if upgrading from pre-metrics schema
        try:
            self.conn.execute("SELECT metrics FROM infons LIMIT 1")
        except sqlite3.OperationalError:
            self.conn.execute("ALTER TABLE infons ADD COLUMN metrics TEXT")
            self.conn.commit()
    ...
    def clear(self) -> None:
        """Delete all rows from every table. Preserves schema."""
        for table in ("infons", "edges", "constraints"):
            try:
    ...
    def get_infon(self, infon_id: str) -> Infon | None:
        row = self.conn.execute(
            "SELECT * FROM infons WHERE infon_id = ? AND pruned = 0", (infon_id,)
        ).fetchone()
    ...
    def get_infons_for_anchor(self, anchor: str, role: str | None = None,
        ...
        if role == "subject":
            sql = "SELECT * FROM infons WHERE subject = ? AND pruned = 0 ORDER BY importance DESC LIMIT ?"
        elif role == "predicate":
            sql = "SELECT * FROM infons WHERE predicate = ? AND pruned = 0 ORDER BY importance DESC LIMIT ?"
        elif role == "object":
            sql = "SELECT * FROM infons WHERE object = ? AND pruned = 0 ORDER BY importance DESC LIMIT ?"
        else:
            sql = ("SELECT * FROM infons WHERE "
                   "(subject = ? OR predicate = ? OR object = ?) "
                   "AND pruned = 0 ORDER BY importance DESC LIMIT ?")
            rows = self.conn.execute(sql, (anchor, anchor, anchor, limit)).fetchall()
            return [_row_to_infon(r) for r in rows]
    ...
    def count_infons(self) -> int:
        row = self.conn.execute("SELECT COUNT(*) FROM infons WHERE pruned = 0").fetchone()
        return row[0]

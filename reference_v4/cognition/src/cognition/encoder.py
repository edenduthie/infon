...
class AnchorProjector:
    ...
    def refresh(self, schema=None):
        """Rebuild the anchor-token map after the schema has been extended.

        Call this after adding new anchors to the schema in place, so that
        subsequent extraction picks up their tokens.
        """
        if schema is not None:
            self.schema = schema
        # Re-run __init__ via a fresh AnchorProjector to regenerate
        # anchor_token_ids against the (possibly extended) schema.
        fresh = AnchorProjector(self.schema, self.tokenizer)
        self.anchor_token_ids = fresh.anchor_token_ids
        return self
...
class Encoder:
    ...
    def refresh_schema(self, schema) -> None:
        """Pick up anchors added to the schema after Encoder construction."""
        if self._projector is not None:
            self._projector.refresh(schema)
        self.anchor_names = list(schema.names)
        self.anchor_types = dict(schema.types)
        self.n_anchors = len(self.anchor_names)

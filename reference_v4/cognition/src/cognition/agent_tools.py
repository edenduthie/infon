...
@tool
def evaluate_logic(expression_json: str) -> str:
    """Evaluate a JSON logical expression against the knowledge graph.

    Use this tool when a natural-language question has structure you can
    decompose — conjunctions, negations, existential claims, conditionals.
    The expression is a JSON object with one of these operators:
      ...
      exists:  {"op":"exists","type":"actor","body":<expr with "?x">}
      forall:  {"op":"forall","type":"actor","body":<expr with "?x">}

    Returns a verdict (SUPPORTS / REFUTES / NOT ENOUGH INFO), a
    Dempster-Shafer mass (supports / refutes / uncertain / theta),
    and a trace of every sub-expression's result.

    Example — "Did Toyota invest in batteries but not acquire CATL?":
      {"op":"and","args":[
        {"op":"triple","s":"toyota","p":"invests","o":"battery"},
        {"op":"not","args":[
           {"op":"triple","s":"toyota","p":"acquires","o":"catl"}]}]}

    theta near 1.0 means "no evidence" — the tool is being honest
    about not knowing. Use list_anchors() to discover the schema.
    """
    import json as _json
    from .logical_tool import evaluate, LogicalExprError

    try:
        expr = _json.loads(expression_json)
    except _json.JSONDecodeError as e:
        return f"ERROR: invalid JSON — {e}"

    try:
        result = evaluate(_cog, expr)
    except LogicalExprError as e:
        return f"ERROR: expression violates the grammar — {e}"

    m = result["mass"]
    lines = [
        f"verdict:    {result['verdict']}",
        f"supports:   {m['supports']:.3f}",
        f"refutes:    {m['refutes']:.3f}",
        f"uncertain:  {m['uncertain']:.3f}",
        f"theta:      {m['theta']:.3f}",
        "",
        "trace (bottom → top):",
    ]
    for step in result["trace"]:
        ...
        mass = step.get("mass", {})
        theta = mass.get("theta", 0.0)
        if op == "triple":
            lines.append(
                f"  {step['query']!r:<55s} → {verdict} (θ={theta:.2f})"
            )
        else:
            lines.append(
                f"  {op} → {verdict} (θ={theta:.2f})"
            )
    return "\n".join(lines)

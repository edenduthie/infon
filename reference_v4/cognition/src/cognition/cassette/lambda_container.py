...
# Copy the cognition package (bundles 17MB SPLADE + heads in cognition/model)
COPY cognition/ ${{LAMBDA_TASK_ROOT}}/cognition/
...
def _write_build_context(ctx_dir: Path, requirements: list[str],
                          base_image: str) -> None:
    """Lay out Dockerfile + requirements + cognition/ + handler.py.

    Everything below this dir is sent to the daemon as the build context
    — keep it small (just what the image needs)."""
    import cognition

    cognition_src = Path(cognition.__file__).parent

    # Drop __pycache__ to avoid stale .pyc files making the image nondeterministic.
    def _ignore(d, names):
        return [n for n in names if n == "__pycache__" or n.endswith(".pyc")]

    dst_cog = ctx_dir / "cognition"
    if dst_cog.exists():
        shutil.rmtree(dst_cog)
    shutil.copytree(cognition_src, dst_cog, ignore=_ignore)

    # handler.py lives inside the cognition package; copy it to the root
    # so the Lambda runtime finds it via CMD ["handler.handler"].
    handler_src = cognition_src / "cassette" / "handler.py"
...
def build_image(
    *,
    ...
    Args:
      image_tag: Docker tag for the built image, e.g. "cognition-ingest:v1".

    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if include_model:
            ...
            if model_dir is None:
                import cognition
                model_dir = Path(cognition.__file__).parent / "model"
            model_dir = Path(model_dir)
            if not model_dir.exists():
                raise FileNotFoundError(f"model dir not found: {model_dir}")
            dst_model = site_packages / "cognition" / "model"
            if dst_model.exists():
        ...
        zip_path = output_dir / "cognition-runtime.zip"
...
def publish_layer(
    *,
    ...
    description = description or f"cognition layer  sha256={sha}"
...
def build_function_zip(output_dir: str | Path = "./build") -> Path:
    ...
    output_dir = Path(output_dir).resolve()
    output_dir.mkdir(parents=True, exist_ok=True)
    zip_path = output_dir / "cognition-handler.zip"
    import cognition
    handler_src = Path(cognition.__file__).parent / "cassette" / "handler.py"
    with zipfile.ZipFile(zip_path, "w",

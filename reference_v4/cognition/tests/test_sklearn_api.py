...
def test_reasoner_is_estimator():
    from cognition.estimator import Estimator
    with tempfile.TemporaryDirectory() as tmp:
        cog = _build_cog(tmp)
        cog.ingest(DOCS)
        cog.consolidate()
...
def test_held_out_infon_split_is_deterministic():
    from cognition.model_selection import held_out_infon_split
    with tempfile.TemporaryDirectory() as tmp:
        cog = _build_cog(tmp)
        cog.ingest(DOCS)
        cog.consolidate()
...
def test_kfold_infon_split_disjoint_and_covering():
    from cognition.model_selection import kfold_infon_split
    with tempfile.TemporaryDirectory() as tmp:
        cog = _build_cog(tmp)
        cog.ingest(DOCS)
        cog.consolidate()

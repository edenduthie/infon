...
from cognition import Cognition, CognitionConfig, Infon, QueryResult
from cognition.category import SchemaDiscovery


@dataclass
class FEVERResult:
    """Result for a single FEVER claim under cognition evaluation."""
    claim_id: str
    claim: str
    gold_label: str
...
class CognitionFEVERRunner:
    """Run cognition system on FEVER claims.

    Strategy:
    1. Ingest relevant Wikipedia pages into cognition (per claim or batch)
    ...
    def evaluate_claim(
        ...
        # Fresh cognition instance per claim (isolation)

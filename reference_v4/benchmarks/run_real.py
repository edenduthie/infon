...
def evaluate_fever_cognition(claims: list[dict], schema_path: Path,
                              condition: str, top_k: int = 20) -> dict:
    """Evaluate cognition on FEVER claims.
    ...
    for i, claim in enumerate(claims):
        ...
        # Fresh cognition instance per claim
...
def evaluate_fever_heads(claims: list[dict], schema_path: Path, top_k: int = 20) -> dict:
    ...
    heads_path = Path(__file__).parent.parent / "cognition" / "src" / "cognition" / "model"
...
def evaluate_hover_cognition(claims: list[dict], schema_path: Path,
                              condition: str, chain_depth: int = 15) -> dict:
    """Evaluate cognition on HoVer claims with hyperedge traversal.
    ...
    for i, claim in enumerate(claims):
        ...
        from cognition.extract import split_sentences
        ...
        # Fresh cognition with consolidation for NEXT edges
        config = CognitionConfig(
            schema_path=str(schema_path),
            db_path=":memory:",
            activation_threshold=0.2,
...
def main():
    ...
    if args.fever:
        ...
        if "fixed" in conditions:
            fever_results["fixed"] = evaluate_fever_cognition(
        ...
        if "discovered" in conditions:
            ...
            fever_results["discovered"] = evaluate_fever_cognition(
    ...
    if args.hover:
        ...
        if "fixed" in conditions:
            hover_results["fixed"] = evaluate_hover_cognition(
        ...
        if "discovered" in conditions:
            ...
            hover_results["discovered"] = evaluate_hover_cognition(
